...
def parse_minibatch_CIRCD1(adjlists_cd, edge_metapath_indices_list_cd, circrna_disease_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_cd, edge_metapath_indices_list_cd)):
        for adjlist, indices, use_mask in zip(adjlists, edge_metapath_indices_list, use_masks[mode]):
            if use_mask:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
                    [adjlist[row[mode]] for row in circrna_disease_batch], [indices[row[mode]] for row in circrna_disease_batch], samples, circrna_disease_batch, offset, mode)
            else:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
                    [adjlist[row[mode]] for row in circrna_disease_batch], [indices[row[mode]] for row in circrna_disease_batch], samples, offset=offset, mode=mode)
            ...
            g_lists[mode].append(g)
            result_indices_lists[mode].append(result_indices)
            idx_batch_mapped_lists[mode].append(np.array([mapping[row[mode]] for row in circrna_disease_batch]))

    return g_lists, result_indices_lists, idx_batch_mapped_lists

def parse_minibatch_CIRCD2(adjlists_cd, edge_metapath_indices_list_cd, circrna_disease_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_cd, edge_metapath_indices_list_cd)):
...
def parse_minibatch_CIRDM3(adjlists_cd, edge_metapath_indices_list_cd, circrna_disease_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_cd, edge_metapath_indices_list_cd)):
...
def parse_minibatch_LNCD(adjlists_cd, edge_metapath_indices_list_cd, lncrna_disease_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_cd, edge_metapath_indices_list_cd)):
        for adjlist, indices, use_mask in zip(adjlists, edge_metapath_indices_list, use_masks[mode]):
            if use_mask:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
                    [adjlist[row[mode]] for row in lncrna_disease_batch], [indices[row[mode]] for row in lncrna_disease_batch], samples, lncrna_disease_batch, offset, mode)
            else:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
                    [adjlist[row[mode]] for row in lncrna_disease_batch], [indices[row[mode]] for row in lncrna_disease_batch], samples, offset=offset, mode=mode)
            ...
            g_lists[mode].append(g)
            result_indices_lists[mode].append(result_indices)
            idx_batch_mapped_lists[mode].append(np.array([mapping[row[mode]] for row in lncrna_disease_batch]))

    return g_lists, result_indices_lists, idx_batch_mapped_lists

def parse_minibatch_CIRDM3_gene(adjlists_cd, edge_metapath_indices_list_cd, circrna_disease_batch, device, samples=None, use_masks=None, offset=None):
    g_lists = [[], []]
    result_indices_lists = [[], []]
    idx_batch_mapped_lists = [[], []]
    for mode, (adjlists, edge_metapath_indices_list) in enumerate(zip(adjlists_cd, edge_metapath_indices_list_cd)):
